...
COOKBOOK = {'onigiri':'210000', 'califonia_roll':'111000', 'gunmaki':'112000', 'sushi':'110020', 'shrimpushi': '110200', 'unagshi':'110002', 'dragon_roll':'211002'\
            , 'combo':'211111'}                                   
# order of ingrediants will be the following, rice, nori, feggs, shrimps, salmon, unagi
DUR = 0.15
...
class Kitchenbot:
    ...
    def recieve_orders(self, foodlist: dict, timelist: dict) -> None:
        ...
        for seat in [f'seat{i}' for i in range(1, 7)]:
            plate_travel_time = 3.5*(int(seat[-1]) - 1) if int(seat[-1]) > 1 else 0
            ...
            elif self.seatordersplate[seat] != foodlist[seat] and self.prevseatordersplate[seat] != foodlist[seat]:
        ...
        for seat in [f'seat{i}' for i in range(1, 7)]:
            if self.seatordersplate[seat] is not None:
                debug(f'Cooking {self.seatordersplate[seat]}')
    ...
    def cook_plate(self, plate: str, seat: str) -> None:
        ...
        for ing_number in range(6):
            intingrd_number = int(recipe[ing_number])
            if intingrd_number != 0:
                for _ in range(intingrd_number):
                ...
                continue
        debug('sleeping before clicking on the mat..')
        sleep(0.6)
    ...
    def checkandcall(self):
        ...
        for ingred, amount in self.inventory.items():
            ...
            elif ingred in ('shrimp', 'salmon', 'unagi'):
                if amount <= 2 and not self.ingred_lacking[ingred]:
                    info(f'Calling for {ingred}..')
                    self.call(ingred)
                elif amount < 2 and self.ingred_lacking[ingred]:
    ...
    def update_inv(self, mode: bool) -> None:
        ...
        if not mode:
            ...
            for ingred, amount in self.inventory.items():
                if amount < 0:
                    critical(f'Shit just hit the fan, {ingred} is currently at the count: {amount}')
                elif amount == 0:
                    info(f'{ingred} has run out, disabling all dishes that needs it.. ')
                    self.ingred_unavilable[ingred] = True
    ...
    def call(self, ingred: str) -> None:
        ...
        toppings = ('shrimp', 'salmon', 'unagi', 'nori', 'feggs')
        ...
        if ingred in toppings:
            ...
            try:
                pyautogui.locateCenterOnScreen(getpath(f'{ingred}_afford.png'), minSearchTime=0.8, grayscale=False, region=(930, 460, 1140, 700))
            ...
            try:
                pyautogui.locateCenterOnScreen(getpath(f'rice_afford.png'), grayscale=False, region=(930, 460, 1140, 700), minSearchTime=0.8)
